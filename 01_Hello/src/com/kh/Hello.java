 ...
 *  - 모든 클래스는 하나의 패키지에 속하며, 패캐지가 선언되지 않는 클래스는 자동적으로 이르없는 패키지에 속하게 된다
*/
package com.kh;
...
public class Hello { // 클래스 명은 클래스 파일명과 동일해야 한다.
	 ...
	 * - 해당 메소드 호출시 메소드의 블럭({})에 해당하는 부분을 실행한다.
	 */
	
	// 실행 메소드(main 메소드) : 자바 프로그램 실행시 java.exe에 의해 가장 먼저 호출한다.
	//모든 클래스가 main 메소드를 가지고 있어야 하는 것은 아니지만 하나의 Java 애플리케이션에는 main 메소드를 포함한 하나의 클래스가 반드시 있어야 한다.
...
- 클래스명 : 대문자로 시작, 여러 개의 단어가 있을 때는 단어 앞자리마다 대문자로 표기.
...
자바 프로그램 실행 단계
 ...
 - 번역된 바이트코드를 JVM의 자바 인터프리터(java.exe)를 이용해 실행한다.
*/
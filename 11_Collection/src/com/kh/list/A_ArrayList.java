 ...
 *  - 컬렉션을사용하면 데이터 추가, 삭제, 정렬 등의 처리가 간단하게 해결되어 자료구조적 알고리즘을 구현할 필요가 없음
 */
...
//		a.method3();
	...
	public void method2() {
		...
		list.set(0, new Person("오은영", "강남", 57));
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (a.equals(list.get(i).getAddr())) {
	...
	public void method3() {
		...
		// 13. 내림차순 정렬
		Collections.reverse(list); //sort 이후 사용해야 내림차순으로 정렬됨
		System.out.println(list);
	}

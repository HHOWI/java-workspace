 ...
 * 		-> Object 클래스에서 제공하는 메서드들을 오버라이딩하여 메서드 재정의가 가능
...
public class Notebook extends Product /* extends Object */ {
	...
	public Notebook(String brand, String pCode, String name, int price, String cpu) {	//순서 지켜야 함
		// 부모 멤버변수 초기화 방법
		// super : 해당 객체의 부모의 객체의 주소를 담고 있음
		
		// 1. super. 을 통해서 부모의 멤버변수에 직접 접근
//		super.brand = brand; // 부모가 private - 직접 접근 불가
...
//		super.price = price; // 부모가 public - 어디서든 접근이 가능
		...
		// 2. 부모의 Setter 메서드를 호출해서 초기화 - 부모 클래스에서 setter 메서드를 제공하면 자식 객체에서 부모의 setter 메서드를 통해서 필드의 초기화가 가능

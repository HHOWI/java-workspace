...
//		a.method1();
	...
	public void method2() {
		boolean result = false;
		List<Student> students = Arrays.asList(new Student("홍수민", 7, "남자", 50, 80), new Student("강원구", 2, "남자", 90, 90),
				new Student("박진실", 15, "여자", 80, 50), new Student("이주연", 18, "여자", 60, 80),
				new Student("최준혁", 14, "남자", 70, 90));
		// 나이가 10살 이상인 학생들이 모두 여자인지 확인하시오.
		// 남학생들 중에 평균이 70점 이상인 학생이 한 명이라도 존재하는지 확인하시오.

		result = students.stream().filter(student -> student.getAge() >= 10)
				.allMatch(student -> student.getGender().equals("여자"));
		System.out.println("나이가 10살 이상인 학생들이 모두 여자입니까? : " + result);
		...
		System.out.println("남학생들 중에 평균이 70점 이상인 학생이 한 명이라도 존재입니까? : " + result);

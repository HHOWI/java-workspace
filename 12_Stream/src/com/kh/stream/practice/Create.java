 ...
 * 
 * 스트림의 종류
 * - java.util.stream 패키지에 존재하고 BaseStream 인터페이스를 부모로 해서 자식 인터페이스들이 사옥 관계를 이루고 있다.
 * 
 * Stream API
 ...
 * 		- 리턴 타입이 Stream 이라면 중간 처리 메서드, 기본 타입이거나, Optional 객체라면 최종 처리 메서드
...
//		c.method2();
	...
	public void method1() {
		...
		// 첫 번째 매개 값 ~ 두 번째 매개 값 까지의 값을 요소로 가지는 스트림 객체를 생성
		stream = IntStream.rangeClosed(1, 10);
		sum = stream.sum();
		System.out.println("rangeClosed의 sun : " + sum);

	}

	/*
	 * 배열로 부터 스트림을 생성하는 방법 - Arrays.stream(배열) 메서드로 스트림을 생성한다. - 각 스트림(Stream,
	 * IntStream, ...)의 of(배열) 메서드로 스트림을 생성한다.
	 */
	public void method2() {
